...
get_locals_test <-
  function(X, pls, g)
  {
    ...
    loadcomu <- get_loads(X.g1, Y1.lvs, blocks)   
    load.g1 <- loadcomu[[1]]
    # gof
    ...
    # calculating loadings and communalities for each class
    loadcomu <- get_loads(X.g2, Y2.lvs, blocks)   
    load.g2 <- loadcomu[[1]]
    # gof
    ...
    for (i in 1:reps)# multigroup permutation
    {
      ...
      loadcomu <- get_loads(X.g2, Y2.lvs, blocks)   
      loads.g2 <- loadcomu[[1]]  
      gofs.g1 <- get_gof(loads.g1^2, R2.g1, blocks, IDM)  
      gofs.g2 <- get_gof(loads.g2^2, R2.g2, blocks, IDM)
